...
test('legacy /sign/comment with an em dash and emoji renders every operation row', async ({
  page,
}) => {
...
test('a vote sign page needs a posting key and offers Continue when logged out', async ({
  page,
}) => {
  await page.goto(
    '/sign/vote?author=ecency&permlink=render-check&weight=10000',
    {
  ...
  await expect(
    page.locator('[data-e2e="login-continue"], a:has-text("Continue")').first(),
  ).toBeVisible();
});
...
test('an unknown operation shows the invalid-data error', async ({ page }) => {
  ...
  await expect(
    page.getByRole('button', { name: /approve|continue|sign/i }),
  ).toHaveCount(0);
});
